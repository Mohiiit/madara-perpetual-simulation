...
import { SPEC_LOCK_MANIFEST, type SpecFileLock, type SpecLockEntry } from "./spec-lock-manifest.js";
...
function getFileLock(entry: SpecLockEntry, fileName: string): SpecFileLock {
  ...
  if (!file) {
    throw new Error(`Spec lock manifest for ${entry.tag} does not include ${fileName}`);
  }
  return file;
}
...
export function resolveSpecCacheDir(override?: string): string {
  return normalizeCacheDir(override || process.env.STARKNET_SPEC_CACHE_DIR || "");
}

export function resolveCachedSpecFilePath(
  specTag: string,
  fileName: string,
  cacheDirOverride?: string,
): string {
  return path.resolve(resolveSpecCacheDir(cacheDirOverride), specTag, fileName);
}
...
async function cacheSingleFile(
  ...
  const targetPath = resolveCachedSpecFilePath(specTag, fileLock.file, cacheDirOverride);
...
export async function ensureSpecCached(options: {
  specTag: string;
  fileNames: string[];
  cacheDirOverride?: string;
}): Promise<void> {
  const entry = getManifestEntry(options.specTag);
  for (const fileName of options.fileNames) {
    await cacheSingleFile(options.specTag, getFileLock(entry, fileName), options.cacheDirOverride);
  }
}
